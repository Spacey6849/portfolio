...
function createTextTexture(
  ...
  const scale = 3; // render at 3x for crisp retina-quality text
  const canvas = document.createElement('canvas');
  const context = canvas.getContext('2d');
  if (!context) throw new Error('Could not get 2d context');

  // Scale up the font size for measurement
  const fontSize = getFontSize(font);
  const scaledFont = font.replace(`${fontSize}px`, `${fontSize * scale}px`);
  ...
  // Return logical (1x) dimensions so the mesh scales correctly
...
class Media {
  ...
  onResize({ screen, viewport }: { screen?: ScreenSize; viewport?: Viewport } = {}) {
    ...
    this.plane.scale.y = (this.viewport.height * (675 * this.scale)) / this.screen.height;
...
class App {
  ...
  onTouchUp(e: MouseEvent | TouchEvent) {
    if (!this.hasDragged && this.onItemClick) {
      // It was a click, not a drag — find which card was clicked
      const clientX = 'changedTouches' in e ? e.changedTouches[0].clientX : e.clientX;
      const clientY = 'changedTouches' in e ? e.changedTouches[0].clientY : e.clientY;
      const clickedIndex = this.getClickedMediaIndex(clientX, clientY);
      if (clickedIndex !== -1) {
        // Map back to original item index (items are duplicated for infinite scroll)
        const originalLength = this.mediasImages.length / 2;
        const originalIndex = clickedIndex % originalLength;
  ...
  getClickedMediaIndex(clientX: number, clientY: number): number {
    ...
    // Convert NDC to world coordinates at z=0 using the camera
    ...
    // Check which media plane contains this point
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    const app = new App(containerRef.current, {
      ...
      scrollSpeed,
      scrollEase,
      onItemClick: (index: number) => onItemClickRef.current?.(index)
    });
    return () => {
      app.destroy();
    };
